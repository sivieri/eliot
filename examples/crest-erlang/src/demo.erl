...
word_frequency() ->
        receive
			...
			{Pid, {"param", "parameters"}} ->
                ...
                word_frequency();
            {Pid, {[{"addresses", Addresses}, {"filename", Filename}, {"limit", Limit}], _}} ->
                AddressList = string:tokens(Addresses, "\r\n"),
                AddressList2 = lists:map(fun(Element) -> {Element, Filename, Limit} end, AddressList),
                Result = lists:foldl(fun(X, AccIn) -> word_called_function(X, AccIn) end, [], AddressList2),
...
idf(Obj, N) ->
		...
		DictList = lists:map(fun(I) ->
									 Address = crest_json:destructure(crest_utils:format("Obj[~p].ip", [I]), Obj),
									 M = crest_json:destructure(crest_utils:format("Obj[~p].total", [I]), Obj),
									 Res = lists:map(fun(J) ->
													   Word = crest_json:destructure(crest_utils:format("Obj[~p].words[~p].word", [I, J]), Obj),
